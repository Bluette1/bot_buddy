...
class BirthdayCommands(commands.Cog):
    def __init__(self, bot, birthday_repo):
        ...
        self.check_birthdays = tasks.loop(hours=1)(self.check_birthdays_task)
        ...
        try:
            ...
            for birthday in all_birthdays:
                ...
                if 0 <= days_until <= days:
                    ...
                    upcoming.append(
                        (user.name, bday.strftime('%B %d'), days_until))
        ...
        try:
            ...
            for birthday in all_birthdays:
                month = datetime.strptime(
                    birthday.birthday, '%Y-%m-%d').strftime('%B')
    ...
    async def check_birthdays_task(self):
        ...
        try:
            ...
            for birthday in birthdays:
                birthday_month_day = datetime.strptime(
                    birthday.birthday, '%Y-%m-%d').strftime('%m-%d')

                if birthday_month_day == today:
                    user = await self.bot.fetch_user(birthday.user_id)
                    message = birthday_messages[hash(
                        str(birthday.user_id)) % len(birthday_messages)]
                    try:
    ...
    def start_check_birthdays(self):
